...
class EkopiecConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    def async_get_options_flow(config_entry):
        """Get options flow."""
        return EkopiecOptionsFlowHandler(config_entry)


class EkopiecOptionsFlowHandler(config_entries.OptionsFlow):
    """Handle options for ekopiec integration."""
    ...
    async def async_step_init(self, user_input=None):
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Optional(
                    "show_temperatures",
                    default=options.get("show_temperatures", True),
                ): cv.boolean,
                vol.Optional(
                    "show_climate",
                    default=options.get("show_climate", True),
                ): cv.boolean,
                vol.Optional(
                    "show_dhw",
                    default=options.get("show_dhw", True),
                ): cv.boolean,
                vol.Optional(
                    "show_switches",
                    default=options.get("show_switches", True),
                ): cv.boolean,
                vol.Optional(
                    "show_fuel",
                    default=options.get("show_fuel", True),
                ): cv.boolean,
                vol.Optional(
                    "show_alarms",
                    default=options.get("show_alarms", False),
                ): cv.boolean,
                vol.Optional(
                    "show_numbers",
